...
AVSValue __cdecl Create_Shader(AVSValue args, void* user_data, IScriptEnvironment* env) {
	return new Shader(
		...
		args[17].AsClip(),			// clip 5
		args[18].AsClip(),			// clip 6
		args[19].AsClip(),			// clip 7
		args[20].AsClip(),			// clip 8
		args[21].AsClip(),			// clip 9
		args[22].AsInt(0),			// width
		args[23].AsInt(0),			// height
		env);						// env is the link to essential informations, always provide it
}
//
...
extern "C" __declspec(dllexport) const char* __stdcall AvisynthPluginInit3(IScriptEnvironment* env, const AVS_Linkage* const vectors) {
	...
	env->AddFunction("Shader", "c[path]s[entryPoint]s[shaderModel]s[precision]i[param1]s[param2]s[param3]s[param4]s[param5]s[param6]s[param7]s[param8]s[param9]s[clip2]c[clip3]c[clip4]c[clip5]c[clip6]c[clip7]c[clip8]c[clip9]c[width]i[height]i", Create_Shader, 0);
