...
static InputTexture* __stdcall FindTexture(std::vector<InputTexture*>* textureList, int clipIndex) {
...
static HRESULT __stdcall ReleaseTexture(InputTexture* obj) {
...
static HRESULT __stdcall RemoveTexture(std::vector<InputTexture*>* textureList, InputTexture* item) {
	textureList->erase(std::remove(textureList->begin(), textureList->end(), item), textureList->end());
	HR(ReleaseTexture(item));
	return S_OK;
}

static HRESULT __stdcall ClearTextures(MemoryPool* pool, std::vector<InputTexture*>* textureList) {
	for (auto const& item : *textureList) {
		if (item->Pool == pool)
			HR(ReleaseTexture(item));
	}
...
static D3DFORMAT __stdcall GetD3DFormat(int precision, bool planar) {
	if (precision == 0)
		return D3DFMT_L8;
	else if (precision == 1) {
		if (planar)
...
static HRESULT __stdcall CopyAviSynthToBuffer(const byte* src, int srcPitch, int clipPrecision, int width, int height, InputTexture* dst, IScriptEnvironment* env) {
	...
	HR(D3DXLoadSurfaceFromMemory(dst->Surface, NULL, NULL, src, GetD3DFormat(clipPrecision, false), srcPitch, NULL, &SrcRect, D3DX_FILTER_NONE, 0));
	return S_OK;
}

static HRESULT __stdcall CopyAviSynthToPlanarBuffer(const byte* srcY, const byte* srcU, const byte* srcV, int srcPitch, int clipPrecision, int width, int height, InputTexture* dst, IScriptEnvironment* env) {
	// Copies source frame into main surface buffer, or into additional input textures
	RECT SrcRect;
	SrcRect.top = 0;
	SrcRect.left = 0;
...
static HRESULT __stdcall CopyBufferToAviSynthInternal(IDirect3DSurface9* surface, byte* dst, int dstPitch, int rowSize, int height, IScriptEnvironment* env) {
